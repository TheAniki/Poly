...
float Rabais::getRabais(Facture f, int code_client) {
	...
	if (le_client->getAge() > 65) rabais += 0.05;
	
	// Rabais basé sur la zone
	if (le_client->getZone().compare("G0X") == 0) rabais += 0.02;
	else if (le_client->getZone().compare("H4L") == 0) rabais += 0.02;
	else if (le_client->getZone().compare("H2X") == 0) rabais += 0.01;
	...
	if (rabais_adhesion > 5) rabais_adhesion = 5;
	rabais += float(rabais_adhesion) / 100;
